...
from agents.crash_analyzer.prompt import *
from datetime import datetime
import time

def run_crash_analyzer(input_msg: str = None):
    """
    Run the crash analysis agent.
    """
    ...
    logger = get_logger("Crash Analysis Agent")
    logger.info("Crash Analysis Agent started.")
    aci = ACI(logger)
    ...
    while main_flag:
        ...
        while retry < 3:
            try:
                ...
                logger.info(f"Tokens - Completion: {response.usage.completion_tokens}, Prompt: {response.usage.prompt_tokens}, Total: {response.usage.total_tokens}")
                break
            ...
            if retry == 3:
                logger.error("LLM API failed 3 times. Exiting.")
                main_flag = False
                break

        epoch += 1
        response_text = response.choices[0].message.content
        logger.info(f"Epoch {epoch} Response:\n{response_text}")
        ...
        if action == "Use_Command":
            ...
            observation = aci.run_command_non_blocking(aci.container, action_input.strip(), 15, update=False)
            content = f"Crash Analysis Output:\n{observation}\n{epoch_prompt}"
            history_processor.extend_msg(messages, response_text, content)

        elif action == "Deduplicate_Crashes":
            observation = aci.run_command_non_blocking(aci.container, action_input.strip(), 10, update=False)
            content = f"Deduplication Result:\n{observation}\n{epoch_prompt}"
            history_processor.extend_msg(messages, response_text, content)
            ...
            output_file = locals()['crash_analyzer_output_file']
            ...
            logger.error("Invalid or failed action.")
