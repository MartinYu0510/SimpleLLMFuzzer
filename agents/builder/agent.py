...
from agents.builder.prompt import *
from datetime import datetime
import time
# Define the main agent loop
def run_builder(input_msg: str=None):
    """
    ...
    # Define logger
    logger = get_logger("Builder Agent")
    ...
    while main_flag:
        # Query the LLM API
        # Check if the LLM query has timed out in 60 seconds. Retry up to 3 times.
        # logger.info(f"Query\n{messages}")
        retry = 0
        while retry < 3:
            try:
        ...
        if action == "Use_Command":
            ...
            history_processor.extend_msg(messages, response_text, content)
        elif action == "Run_Compiler":
            response_format_error_flag = False
            observation, exit_code = aci.run_command_with_timeout(aci.container, action_input.strip(), -1, update=False)
            content = f"Observation: Your command exited with code {exit_code}. {observation}\n(Open file: {aci.open_file})\n(Current directory: {aci.working_dir})\nbash-$" + epoch_prompt + Self_critic_message
            history_processor.extend_msg(messages, response_text, content)
        elif action == "Respond_to_Human":
            response_format_error_flag = False
            logger.info(f"Response: {action_input}")
            output_file = locals()['builder_output_file']
