...
from agents.dictionary.prompt import *
from datetime import datetime
import time

# Define the main agent loop
def run_dictionary_generator(input_msg: str = None):
    """
    Run the dictionary generation agent.
    """
    ...
    # Define logger
    logger = get_logger("Dictionary Generator Agent")
    logger.info("Dictionary Generator agent started.")
    ...
    while main_flag:
        ...
        if action == "Use_Command":
            ...
            content = f"Target Analysis:\n{observation}\n" + epoch_prompt
            history_processor.extend_msg(messages, response_text, content)

        elif action == "Generate_Dictionary":
            response_format_error_flag = False
            observation = aci.run_command_non_blocking(aci.container, action_input.strip(), 10, update=False)
            content = f"Generated Dictionary:\n{observation}\n" + epoch_prompt
            history_processor.extend_msg(messages, response_text, content)

        elif action == "Store_Dictionary":
            response_format_error_flag = False
            observation = aci.run_command_non_blocking(aci.container, action_input.strip(), 5, update=False)
            content = f"Dictionary stored successfully.\n" + epoch_prompt
            history_processor.extend_msg(messages, response_text, content)

        elif action == "Optimize_Dictionary":
            response_format_error_flag = False
            observation = aci.run_command_non_blocking(aci.container, action_input.strip(), 10, update=False)
            content = f"Dictionary Optimization Completed:\n{observation}\n" + epoch_prompt
            history_processor.extend_msg(messages, response_text, content)

        elif action == "Respond_to_Human":
            response_format_error_flag = False
            logger.info(f"Response: {action_input}")
            output_file = locals()['dictionary_output_file']
